...
const calculatorDisplay = container.querySelector('.calculator-display')
...
const div = new Op('div', '÷', (a, b) => (a == 0 || b == 0) ? 'ERROR' : a / b)
...
function addNegToString() {
	if (arr[1] != undefined) {
		if (arr[2] == undefined) arr[2] = 0
		if (!arr[2].includes('.')) arr[2] = `-${arr[2]}`
	}
	
	if (arr[1] == undefined) {
		if (arr[0] == undefined) arr[0] = 0
		if (!arr[0].includes('.')) arr[0] = `-${arr[0]}`
	}
...
function  operate() {	
	...
	if (typeof arr[2] == String) arr[2].toString()
}
